...
GameScene::GameScene()
{
	...
	light3 = globals::device->getSceneManager()->addLightSceneNode(
		0,
		vector3df(25, 25, 25),
		SColor(255,0,0,255));
	light3->setRadius(200);
}
...
Scene::E_SCENE_RETURN_CODE GameScene::run()
{

	light->setPosition(camera->getPosition() + rand() % 300 - 150);
	light3->setPosition(camera->getPosition() + rand() % 300 - 150);
	for (unsigned i = 0; i < Object::allObjects.size(); i++)
	{
